        ...
        <AnimatePresence mode="popLayout" custom={dir}>
          <motion.div
            ...
            <div className="relative z-10">
              <div className="flex gap-1 mb-5 text-gold text-lg" role="img" aria-label={`${t.stars} / 5`}>
...
export default function HomePage({ lang }: { lang: Locale }) {
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(reviewSchema(lang)) }} />
      {/* ── HERO ── */}
      <section ref={heroRef} className="relative h-screen overflow-hidden">
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }}
            ...
            <div className="flex flex-col justify-center">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}
                ...
                <div>
                  ...
                  <p className="font-inter text-[11px] text-cream/40">{c.hero.trust}</p>
                </div>
              </motion.div>
            </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
            ...
            <span className="font-grotesk text-[9px] uppercase tracking-[0.35em] text-text-muted rotate-90 origin-center translate-y-10">{c.hero.scroll}</span>
          </motion.div>
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-5 sm:px-8 md:px-16">
          <Reveal className="mb-20">
            <span className="font-grotesk text-xs font-medium uppercase tracking-[0.2em] text-gold flex items-center gap-3 mb-5">
              <span className="w-8 h-px bg-gold" />{c.problems.eyebrow}
            </span>
            <h2 className="font-grotesk font-bold text-[clamp(36px,5vw,80px)] text-cream leading-[0.95] tracking-[-0.03em] max-w-2xl">
              {c.problems.headA}<br /><span className="text-gradient-gold">{c.problems.headB}</span>
            </h2>
          </Reveal>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {c.problems.items.map((item, i) => {
              const offset = ["lg:mt-0", "lg:mt-16", "lg:mt-8"][i];
              const color = ["from-red-950/20 to-transparent", "from-amber-950/20 to-transparent", "from-orange-950/15 to-transparent"][i];
              return (
                <Reveal key={item.num} delay={i * 0.12} className={offset}>
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 md:px-16">
          <Reveal className="mb-16">
            <h2 className="font-grotesk font-bold text-[clamp(36px,5vw,72px)] text-cream leading-[0.92] tracking-[-0.03em]">
              {c.testimonials.headA}<br /><span className="text-gradient-gold">{c.testimonials.headB}</span>
            </h2>
          </Reveal>
      ...
      <section className="py-16 md:py-32 lg:py-44 bg-[#0D0D0D] relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 md:px-16">
          <Reveal className="mb-20">
            <span className="font-grotesk text-xs font-medium uppercase tracking-[0.2em] text-gold flex items-center gap-3 mb-5"><span className="w-8 h-px bg-gold" />{c.services.eyebrow}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {c.services.cards.map((card, i) => (
              <Reveal key={card.num} delay={i * 0.1}>
                <Link href={localize(card.href, lang)} className="block h-full group">
          ...
          <Reveal className="mt-10 text-center">
            <p className="font-inter text-sm text-text-muted mb-4">{c.services.footNote}</p>
            <Link href={localize("/services", lang)}
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 md:px-16 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24 items-start">
            <div className="md:sticky md:top-32">
              <Reveal>
                <span className="font-grotesk text-xs font-medium uppercase tracking-[0.2em] text-gold flex items-center gap-3 mb-6"><span className="w-8 h-px bg-gold" />{c.why.eyebrow}</span>
                <h2 className="font-grotesk font-bold text-[clamp(40px,5vw,80px)] text-cream leading-[0.92] tracking-[-0.03em] mb-8">
                  {c.why.line1}<br />{c.why.line2}<br /><span className="text-gradient-gold">{c.why.line3}</span>
                </h2>
                <p className="font-cormorant text-xl text-cream/40 font-light italic leading-relaxed max-w-sm mb-10">
                  {c.why.italic}
                </p>
                <Link href={localize("/about", lang)} className="inline-flex items-center gap-2 font-grotesk text-sm font-semibold text-gold/60 hover:text-gold transition-colors border-b border-gold/20 hover:border-gold pb-1">
                  {c.why.story}
                </Link>
              </Reveal>
            </div>
            <div className="space-y-0">
              {c.why.pillars.map((pillar, i) => (
      ...
      <section className="py-16 md:py-24 bg-[#080808] relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 md:px-16">
          <Reveal className="mb-12 md:mb-16 text-center">
            <span className="font-grotesk text-[10px] font-bold uppercase tracking-[0.25em] text-gold/50 block mb-4">{c.trust.eyebrow}</span>
            <h2 className="font-grotesk font-bold text-[clamp(28px,4vw,56px)] text-cream tracking-[-0.03em]">
              {c.trust.headA}<br /><span className="text-gradient-gold">{c.trust.headB}</span>
            </h2>
          </Reveal>
